...
class UploadManager(QObject):
    ...
    def __init__(self, client: FichierClient, cat_manager: CatManager, existing_files: dict, folder_ids_by_name: dict,
    ...
    def upload_next_file(self):
        ...
        # Réinitialiser la progression du fichier courant
        self.current_file_progress = 0
    ...
    def on_file_progress(self, percent: int):
        """Met à jour la progression du fichier courant et émet la progression globale"""
        self.current_file_progress = percent
        if self.total_files > 0:
            # Calculer la progression globale en tenant compte des fichiers terminés et du courant
            global_progress = int(
                (self.files_done / self.total_files * 100) +
                (percent / (100 * self.total_files))
            )
            self.progress.emit(global_progress)
            # Également émettre la progression du fichier individuel
