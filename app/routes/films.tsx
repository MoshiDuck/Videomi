    ...
    useEffect(() => {
        const fetchFiles = async () => {
            ...
            try {
                ...
                // AUTO-MATCHING TMDB pour les films non identifiés
                if (config && config.tmdbApiKey) {
                    const { searchMoviesOnTMDB, getMovieDetailsFromTMDB } = 
                        ...
                        if (!file.source_id || !file.source_api) {
                            try {
                                ...
                                const searchResult = await searchProgressively(
                                    words,
                                    (query) => searchMoviesOnTMDB(query, config.tmdbApiKey!, 10),
                                    extractedYear
                                );
                                
                                if (searchResult) {
                                    const movieDetails = await getMovieDetailsFromTMDB(searchResult.result.source_id, config.tmdbApiKey!);
                                    if (movieDetails) {
                                        const metadata = {
                                            source_api: 'tmdb',
                                            source_id: searchResult.result.source_id,
                                            title: movieDetails.title,
                                            year: movieDetails.release_date ? parseInt(movieDetails.release_date.substring(0, 4)) : null,
                                            thumbnail_url: movieDetails.poster_path ? `https://image.tmdb.org/t/p/w500${movieDetails.poster_path}` : null,
                                            genres: movieDetails.genres?.map(g => g.name) || [],
                                            collection_id: movieDetails.belongs_to_collection?.id?.toString() || null,
                                            collection_name: movieDetails.belongs_to_collection?.name || null,
