// INFO : app/routes/others.tsx — contenu uniquement ; layout _app fournit Navigation + AuthGuard.
...
export default function OthersRoute() {
    ...
    const [selectedCategory, setSelectedCategory] = useState<FileCategory>('others');
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [others, setOthers] = useState<FileItem[]>([]);

    const handleFileDeleted = useCallback((fileId: string) => {
        setOthers((prev) => prev.filter((f) => f.file_id !== fileId));
    }, []);
                    ...
                    {others.length === 0 && !loading ? (
                        <div style={{
                            ...
                            <div style={{ 
                                ...
                                {t('emptyStates.noOthersDescription')}
                            </div>
                            <button
