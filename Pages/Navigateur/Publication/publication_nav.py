...
class Publication(QWidget):
    ...
    def __init__(self, firebase_auth: FirebaseAuth, client: FichierClient, db_manager: DatabaseManager, catalogue: Catalogue):
        ...
        self.sync_thread.finished_sync.connect(self._on_sync_finished, Qt.ConnectionType.QueuedConnection)
    ...
    def _on_all_done(self):
        ...
        for path, link in self.uploaded_links.items():
            ...
            title = item.text(2)
            self.db.insert_file(category, title, link, '', '', '{}', '')  # Insère en base
