...
class _BackgroundCallable(QRunnable):
    """QRunnable utilitaire pour exécuter une callable dans le thread-pool."""
...
class Lecteur(BaseFenetre):
    def __init__(self, stream_urls: List[str], taille_ecran: Optional[QSize] = None):
        ...
        # Utilisation de OSDetector pour trouver l'exécutable MPV
        os_detector = OSDetector()
        mpv_exe_path = os_detector.find_mpv()
        if not mpv_exe_path:
            logger.warning("MPV executable introuvable — MPVController risque d'échouer")
    ...
    def changeEvent(self, event):
        """Gère les changements d'état de la fenêtre"""
        super().changeEvent(event)

        if event.type() == event.Type.WindowStateChange:
            if self.windowState() & Qt.WindowState.WindowMinimized:
                # Fenêtre minimisée : cacher les barres
                if hasattr(self, 'top_bar') and self.top_bar.isVisible():
                    self.top_bar.hide()
                if hasattr(self, 'bottom_bar') and self.bottom_bar.isVisible():
                    self.bottom_bar.hide()
                # Arrêter les timers
                if self.hide_bar_timer and self.hide_bar_timer.isActive():
                    self.hide_bar_timer.stop()
                if self._cursor_timer and self._cursor_timer.isActive():
                    self._cursor_timer.stop()
            else:
                # Fenêtre restaurée : redémarrer le timer de curseur
                if self._cursor_timer and not self._cursor_timer.isActive():
                    self._cursor_timer.start()

    # ---------------- Helpers ----------------
    @staticmethod
    def _resolve_mpv_exe() -> Optional[Path]:
        """Retourne le Path vers l'exécutable mpv via OSDetector."""
        try:
            os_detector = OSDetector()
            return os_detector.find_mpv()
        except Exception:
    ...
    # Dans la classe Lecteur, modifiez la méthode _check_cursor_and_show_hide
    @safe_slot
    def _check_cursor_and_show_hide(self) -> None:
        """Affiche/masque les barres en fonction du curseur et de l'état de la fenêtre"""
        # Ne rien faire si la fenêtre est minimisée
        if self.windowState() & Qt.WindowState.WindowMinimized:
            return
        ...
        try:
            ...
            # Vérifier si le curseur est sur le menu des sous-titres
            bls = getattr(self.bottom_bar, 'barLangueSub', None)
            bls_visible = bls and bls.isVisible()
            bls_over = False
            if bls_visible:
                try:
                    bls_geo = bls.geometry()
                    bls_over = bls_geo.contains(cursor_pos)
                except Exception:
                    logger.exception("Erreur vérification géométrie barLangueSub")

            threshold = 30

            if (cursor_pos.y() <= main_geo.y() + threshold) or top_over or bls_over:
                if _safe_getattr(self, "top_bar", None) and not self.top_bar.isVisible():
                    self.top_bar.setVisible(True)
                self.hide_bar_timer.start()

            elif (cursor_pos.y() >= main_geo.y() + main_geo.height() - threshold) or bottom_over or bls_over:
                if _safe_getattr(self, "bottom_bar", None) and not self.bottom_bar.isVisible():
                    self.bottom_bar.setVisible(True)
                self.hide_bar_timer.start()

            else:
                if not top_over and not bottom_over and not bls_over:
                    if not self.hide_bar_timer.isActive():
                        self.hide_bar_timer.start()
                else:
                    if self.hide_bar_timer.isActive():
                        self.hide_bar_timer.stop()
        except Exception:
            logger.exception("_check_cursor_and_show_hide erreur globale")

    # Modifiez également la méthode cacher_barre
    @safe_slot
    def cacher_barre(self) -> None:
        try:
            ...
            # Vérifier si le curseur est sur le menu des sous-titres
            bls = getattr(self.bottom_bar, 'barLangueSub', None)
            on_bls = bls and bls.isVisible() and bls.geometry().contains(pos)

            if not on_top and not on_bottom and not on_bls:
